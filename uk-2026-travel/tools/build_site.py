...
def write(name,title,body,active='',day=0):
    free_day=min(7,max(2,day or 2))
    nav=[('index.html','⌂','首頁','home',''),(f'day-{day or 1}.html','▤','行程','day',' data-today="schedule"' if not day else ''),(f'day-{free_day}.html?mode={"airport" if free_day==7 else "day"}#free-time','◷','自由時間','free',' data-today="free"' if not day else ''),('hotels.html','⌑','飯店','hotels',''),('guide.html','⋯','更多','guide','')]
...
for d in I['days']:
    ...
    for s in d['stops']:
        body+='<li><strong>'+e(s['name'])+'</strong>'+('<p>'+e(s['note'])+'</p>' if s['note'] else '')+(f'<a href="#{s["guide"]}">必看與逛法 →</a>' if s['guide'] else '')+'</li>'
    ...
    if guides:body+='<section id="field-guide"><div class="section-heading"><p class="eyebrow">必看、拍照與值得買的東西</p><h2>景點怎麼逛最順？</h2><p>點選景點，依你剩下的時間選一條逛法。</p></div>'+''.join(guide_html(g) for g in guides)+'</section>'
