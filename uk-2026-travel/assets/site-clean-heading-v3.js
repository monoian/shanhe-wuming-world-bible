...
const labels={shop:'好逛的店',food:'吃喝',gift:'紀念品'};
const typeName=t=>({restaurant:'餐廳',cafe:'咖啡店',fast_food:'速食',pub:'酒吧',bar:'酒吧',gift:'禮品店',souvenir:'紀念品店',department_store:'百貨',mall:'購物中心',supermarket:'超市',clothes:'服飾',shoes:'鞋店',books:'書店',jewelry:'珠寶',beauty:'美妝',chemist:'藥妝',convenience:'便利商店',bakery:'烘焙店'}[t]||'商店');
...
let radius=800,category='shop',position=null,places=[],watchId=null,lastQuery=null,busy=false;
...
const panel=document.createElement('aside');panel.id='nearbyExplorePanel';panel.className='nearby-panel';panel.setAttribute('aria-hidden','true');panel.innerHTML='<div class="nearby-head"><div><small>即時定位</small><h2>📍 附近探索</h2></div><button class="nearby-close" type="button" aria-label="關閉">×</button></div><div class="nearby-body"><p class="nearby-street" id="nearbyStreet">按「取得我的位置」查看目前街道附近。</p><div class="nearby-actions"><button id="nearbyLocate" type="button">取得我的位置</button><button id="nearbyLive" type="button">開啟即時定位</button></div><fieldset class="nearby-radii"><legend>搜尋距離</legend><button type="button" data-radius="300">300m</button><button type="button" data-radius="800" class="active">800m</button><button type="button" data-radius="1500">1.5km</button></fieldset><div class="nearby-tabs" role="tablist"><button type="button" data-category="shop" class="active">好逛的店</button><button type="button" data-category="food">吃喝</button><button type="button" data-category="gift">紀念品</button></div><div id="nearbyStatus" class="nearby-status">定位資料只用於本次搜尋，網站不會儲存。</div><div id="nearbyResults" class="nearby-results"></div><p class="nearby-privacy">GPS 由瀏覽器取得；附近店家查詢會將座標送至 OpenStreetMap 公共服務。</p></div>';
document.body.append(button,panel);
const $=s=>panel.querySelector(s),$$=s=>[...panel.querySelectorAll(s)],status=$('#nearbyStatus'),results=$('#nearbyResults'),street=$('#nearbyStreet'),live=$('#nearbyLive');
...
const classify=t=>{if(t.shop==='gift'||t.shop==='souvenir'||t.shop==='arts'||t.shop==='craft')return'gift';if(t.amenity&&['restaurant','cafe','fast_food','pub','bar','food_court','ice_cream'].includes(t.amenity)||t.shop==='bakery')return'food';return'shop'};
...
function render(){
 ...
 results.innerHTML=list.map(x=>{const q=encodeURIComponent(x.name),g='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(x.lat+','+x.lon)+'&query_place_id='+q,a='https://maps.apple.com/?ll='+x.lat+','+x.lon+'&q='+q;return '<article class="nearby-card"><div class="nearby-card-head"><div><h3>'+E(x.name)+'</h3><span>'+E(x.type)+'</span></div><b>'+ (x.distance<1000?x.distance+'m':(x.distance/1000).toFixed(1)+'km') +'</b></div><p>'+E(x.address)+'</p><div><a target="_blank" rel="noopener" href="'+g+'">Google Maps</a><a target="_blank" rel="noopener" href="'+a+'">Apple Maps</a></div></article>'}).join('');
}
...
async function query(lat,lon){
 ...
 const q='[out:json][timeout:25];(nwr(around:'+radius+','+lat+','+lon+')[shop];nwr(around:'+radius+','+lat+','+lon+')[amenity~"^(restaurant|cafe|fast_food|pub|bar|food_court|ice_cream)$"];);out center tags;';
...
function found(p,force=false){
 position={lat:p.coords.latitude,lon:p.coords.longitude};status.textContent='已定位，正在更新附近資訊…';reverse(position.lat,position.lon);
 if(force||!lastQuery||distance(position.lat,position.lon,lastQuery.lat,lastQuery.lon)>=75)query(position.lat,position.lon);
}
function locate(){if(!navigator.geolocation){status.textContent='此瀏覽器不支援定位。';return}status.textContent='正在等待位置授權…';navigator.geolocation.getCurrentPosition(p=>found(p,true),e=>{status.textContent=e.code===1?'未允許定位；請在瀏覽器網站設定中開啟位置權限。':'暫時無法取得位置，請到戶外或稍後重試。'},{enableHighAccuracy:true,timeout:15000,maximumAge:30000})}
$('#nearbyLocate').addEventListener('click',locate);
live.addEventListener('click',()=>{if(watchId!==null){navigator.geolocation.clearWatch(watchId);watchId=null;live.textContent='開啟即時定位';live.classList.remove('active');status.textContent='已停止即時定位。';return}if(!navigator.geolocation){status.textContent='此瀏覽器不支援定位。';return}watchId=navigator.geolocation.watchPosition(p=>found(p),()=>status.textContent='即時定位暫時無法更新。',{enableHighAccuracy:true,timeout:20000,maximumAge:10000});live.textContent='停止即時定位';live.classList.add('active')});
