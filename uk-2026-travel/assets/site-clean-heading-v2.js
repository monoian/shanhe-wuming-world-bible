...
async function initAttractions(){
 ...
 if(!q('link[data-attraction-css]')){const link=document.createElement('link');link.rel='stylesheet';link.href='assets/attractions.css?v=clean-heading-20260731-2';link.dataset.attractionCss='';document.head.appendChild(link)}
 const content=q('.content-section');if(!content)return;
 try{
  const file=dayNumber<=4?'assets/attractions-2-4.json':'assets/attractions-5-7.json';
  const res=await fetch(file+'?v=clean-heading-20260731-2',{cache:'no-store'});if(!res.ok)throw new Error('attraction data');
