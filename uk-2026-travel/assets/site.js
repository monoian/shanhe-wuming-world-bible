...
function render(){
 ...
 results.innerHTML=list.map(x=>{const search=[x.name,x.address!=='地址請開啟地圖查看'?x.address:''].filter(Boolean).join(' '),q=encodeURIComponent(search),o='https://www.openstreetmap.org/'+x.osmType+'/'+x.osmId,g='https://www.google.com/maps/dir/?api=1&destination='+q+'&travelmode=walking',a='https://maps.apple.com/?daddr='+q+'&dirflg=w';return '<article class="nearby-card"><div class="nearby-card-head"><div><h3>'+E(x.name)+'</h3><span>'+E(x.type)+'</span></div><b>'+ (x.distance<1000?x.distance+'m':(x.distance/1000).toFixed(1)+'km') +'</b></div><p>'+E(x.address)+'</p><div><a class="nearby-exact" target="_blank" rel="noopener" href="'+o+'">精確位置</a><a target="_blank" rel="noopener" href="'+g+'">Google 導航</a><a target="_blank" rel="noopener" href="'+a+'">Apple 導航</a></div></article>'}).join('');
}
