...
async function initAttractions(){
 ...
 if(!q('link[data-attraction-css]')){const link=document.createElement('link');link.rel='stylesheet';link.href='assets/attractions.css?v=magazine-20260731-1';link.dataset.attractionCss='';document.head.appendChild(link)}
 const content=q('.content-section');if(!content)return;
 try{
  const file=dayNumber<=4?'assets/attractions-2-4.json':'assets/attractions-5-7.json';
  const res=await fetch(file+'?v=magazine-20260731-1',{cache:'no-store'});if(!res.ok)throw new Error('attraction data');
  ...
  section.innerHTML=`<div class="section-heading attraction-heading"><div><span class="eyebrow">附近逛街・紀念品</span><h2>景點周遭可以逛什麼、買什麼</h2></div><p>改用旅遊雜誌式大圖呈現，重點只保留附近可逛店家與推薦紀念品。</p></div><div class="attraction-grid">${cards}</div><p class="attraction-note">店家營業時間、庫存與團體自由時間可能異動，實際以領隊集合時間及現場公告為準。</p>`;
